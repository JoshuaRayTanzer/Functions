...
check=c("kamila","EnvStats","tidyverse","shiny","shinyMatrix",
        "shinyRadioMatrix","psych","ggplot2","survival")
flag=0
...
Est_Mod=function(mod=NA,
  ...
  if(plot_type=="X_Range"|plot_type=="x_range"|plot_type=="X_range"|plot_type=="x_Range"){
    ...
    if(is.null(x_effects)){
      print(noquote("Error: x_effects must be specified as a list object with each entry containing four inputs: x variable name, value input in the L matrix, numeric information (if power, 1 if linear, 2 if quadratic, and so on; if spline, NA for the main effect, the knot value if spline coefficient), and type of effect (power or spline),!"))
    }
  ...
  if(na_sum==0){
    ...
    s=summary(mod)
    lme4=ifelse("lmerMod"%in%c(class(mod))|"glmerMod"%in%c(class(mod)),1,0)
    glmmTMB=ifelse("glmmTMB"%in%class(mod),1,0)
    cox=ifelse("coxph"%in%class(mod),1,0)
    rma_mv=ifelse("rma"%in%class(mod),1,0)
    glm=ifelse("lm"%in%class(mod),1,0)
  ...
  if(raw_mod==0){
    ...
    x_flag2=matrix(0,nrow=nrow(L),ncol=ncol(L))
    x_pow=matrix(0,nrow=nrow(L),ncol=ncol(L))
    x_coef=L
  }
  ...
  if(plot_type=="X_Range"|plot_type=="x_range"|plot_type=="X_range"|plot_type=="x_Range"){
    for(i in 1:length(x_effects)){
      for(j in 1:nrow(L)){
        for(k in 1:ncol(L)){
          if(x_effects[[i]][1]%in%split_text[j,]){
            L_view[j,k]=ifelse(x_effects[[i]][3]==1,
                               paste(as.numeric(L_view[j,k])/as.numeric(x_effects[[i]][2]),paste(ifelse(is.na(x_effects[[i]][4]),"",paste("Spline_",x_effects[[i]][4],"_",sep="")),"x",sep=""),sep=""),
                               paste(as.numeric(L_view[j,k])/as.numeric(x_effects[[i]][2]),paste(ifelse(is.na(x_effects[[i]][4]),"",paste("Spline_",x_effects[[i]][4],"_",sep="")),"x^",x_effects[[i]][3],sep=""),sep=""))
            x_flag[j,k]=ifelse(is.na(x_effects[[i]][4]),1,as.numeric(x_effects[[i]][4]))
            x_flag2[j,k]=ifelse(is.na(x_effects[[i]][4]),1,2)
            x_pow[j,k]=as.numeric(x_effects[[i]][3])
            x_coef[j,k]=as.numeric(x_coef[j,k])/as.numeric(x_effects[[i]][2])
          }
        }
      }
    }
  ...
  if(plot_type=="X_Range"|plot_type=="x_range"|plot_type=="X_range"|plot_type=="x_Range"){
    ...
    knots=c(x_flag[,1])[c(1:nrow(x_flag2))[x_flag2[,1]==2]]
    spl_nam=coef_nam[c(1:nrow(x_flag2))[x_flag2[,1]==2]]
    
    p=1
    for(e in 1:ncol(L)){
      for(f in 1:x_range_breaks){
        for(g in 1:nrow(L)){
          if(coef_nam[g]%in%spl_nam){
            knot=which(coef_nam[g]==spl_nam)
            x_mult=ifelse(x_use[f]<knots[knot],0,(x_use[f]-knots[knot])^x_pow[g,1])
          }else{
            x_mult=x_use[f]^x_pow[g,1]
          }
          Lp[g,p]=x_coef[g,e]*x_mult
          if(f==x_range_breaks){
    ...
    for(e in 1:ncol(L)){
      for(f in 1:length(x_output)){
        for(g in 1:nrow(L)){
          if(coef_nam[g]%in%spl_nam){
            knot=which(coef_nam[g]==spl_nam)
            x_mult=ifelse(x_output[f]<knots[knot],0,(x_output[f]-knots[knot])^x_pow[g,1])
          }else{
            x_mult=x_output[f]^x_pow[g,1]
          }
          L2[g,p]=x_coef[g,e]*x_mult
          if(f==x_range_breaks){
            split_end=c(split_end,p)
          }
          if(g==nrow(L)){
...
inspect_mods=function(mod=NA,
  ...
  if("QQ"%in%Plots|"qq"%in%Plots|"Q-Q"%in%Plots|"q-q"%in%Plots|"Q"%in%Plots|"q"%in%Plots){
    QQ=T
  }
  if("Hist"%in%Plots|"hist"%in%Plots|"Histogram"%in%Plots|"histogram"%in%Plots|"H"%in%Plots|"h"%in%Plots){
    Hist=T
  }
  if("X_Var"%in%Plots|"x_var"%in%Plots|"X_var"%in%Plots|"x_Var"%in%Plots|"X"%in%Plots|"x"%in%Plots|"Var"%in%Plots|"var"%in%Plots|"V"%in%Plots|"v"%in%Plots){
    X_var=T
  }
  ...
  if(num_mods==1){
    ...
    nline_blurb=data.frame("Number lines for interpretation of model fit metrics")
    colnames(nline_blurb)=""
    rownames(nline_blurb)=""
    print(nline_blurb)
    
    nline=data.frame(
      rbind("For the -2LL, AIC, and BIC:                  ",
            "Better                                  Worse",
            "<----------|----------|----------|---------->",
            "(Negative             0             (Positive",
            "numbers)                             numbers)")
    ...
    nline_blurb=data.frame("Model Results")
    colnames(nline_blurb)=""
    rownames(nline_blurb)=""
    print(nline_blurb)
    ...
    for(m in 1:num_mods){
      s=summary(mod[[m]])
      lme4=ifelse("lmerMod"%in%c(class(mod[[m]]))|"glmerMod"%in%c(class(mod[[m]])),1,0)
      glmmTMB=ifelse("glmmTMB"%in%class(mod[[m]]),1,0)
      cox=ifelse("coxph"%in%class(mod[[m]]),1,0)
      rma_mv=ifelse("rma"%in%class(mod[[m]]),1,0)
      glm=ifelse("lm"%in%class(mod[[m]]),1,0)
      ...
      if(glmmTMB==1){
        ...
        fit[,m]=c(nrow(summary(mod[[m]])$coefficients$cond),
                  ...
                  BIC(mod[[m]]),
                  -2*logLik(mod[[m]]),
                  cor(predict(mod[[m]],re.form=NA),predict(mod[[m]],re.form=NA)+res)^2)
        if(X_flag==1){
          X[[m]]=mod[[m]]$frame
          X[[m]]=X[[m]][,2:ncol(X[[m]])]
        }
...
#Variables that are in the model more than once as powers of itself
#(e.g., X and X^2) can be designated as the same variable and what the power is

#3). Splines

#Varuables that are in the model more than once as splines
#can be designated as the same variable and where knots are
...
L_Designer=function(mod,
  ...
  if(type=="any"|type=="Any"|type=="a"|type=="A"){
    Design=matrix(nrow=length(L_inputs(mod)$Coefficient),ncol=3)
    
    colnames(Design)=c("Variable Name","Power","Knot")
    row.names(Design)=c(L_inputs(mod)$Coefficient)
    Design[,1]=c(L_inputs(mod)$Coefficient)
    Design[,2]=1
    
    
    ui <- fluidPage(
      
      titlePanel("Input information on variable relationships"),
      mainPanel(
        width=20,
        tags$h4(strong("If two variables are the same (e.g., linear effect, quadratic effect; linear change, spline), edit their names to be the same and specify the power and what value the knot begins"
        )),
        tags$h5(strong("(So for powers, 1 for linear, 2 for quadratic, 3 for cubic, and so on... If  you have a splines from 3-4 and then again from 4 to the max; the first knot would be 3, and the second 4)")),
        tags$h6(strong("(Leave blank if effect is not a spline)")),
        matrixInput("sample",value = Design,
    ...
    server <- function(input, output, session) {  
      ...
      text <- observeEvent(input$go, {
        Design2=rbind(c("Type:","'Any'","''"),Design)
        Design2=ifelse(is.na(Design2)|Design2=="",as.character("NA"),Design2)
        Design2=cbind(Design2,c(as.character(""),c(L_inputs(mod)$Coefficient)))
        Design<<-Design2
        for(p in 1:4){
          if(p==1){
            out=paste("Design=cbind(c(",noquote(paste(Design2[1:nrow(Design2),p],collapse="','")),"),",sep="'")
          }else if(p==2|p==3){
            col=paste("c(",noquote(toString(Design2[1:nrow(Design2),p])),"),",sep="")
            out=paste(out,col,sep='\n')
          }else{
            col=paste("c(",noquote(paste(Design2[1:nrow(Design2),p],collapse="','")),"))",sep="'")
            out=paste(out,col,sep='\n')
          }
        }    
        
        cat(paste("","","Copy and paste the following into R to backup your work!",
                  "","",out,"","",sep='\n'))    
      })
...
#x_range is a name to be plotted across a range of values
  ...
  if(type=="General"|type=="general"|type=="G"|type=="g"|type=="Gen"|type=="gen"){
    ...
    if(is.null(Design)){
      ...
      server <- function(input, output, session) {  
        data <- observeEvent(input$go, {
          all={input$sample}
          outmat=all[2:nrow(all),1:ncol(all)]
          if(is.matrix(outmat)==F){
            outmat=t(as.matrix(outmat))
          }
      ...
      if(x_range==F){
        if(Design[1,2]=="Splines"|Design[1,2]=="Powers"|Design[1,2]=="'Splines'"|Design[1,2]=="'Powers'"|
           Design[1,2]=="Any"|Design[1,2]=="'Any'"){
          m=data.frame(matrix(nrow=(length(unique(Design[,1]))),
                              ncol=Est_Num))
          colnames(m)=c(paste("Estimate ",1:Est_Num,sep=""))
          row.names(m)=c("Estimate Name",c(unique(Design[2:nrow(Design),1])))
          m=as.matrix(m)
          ...
          server <- function(input, output, session) {  
            data <- observeEvent(input$go, {
              ...
              for(p in 1:ncol(outmat)){
                outmat[,p]=as.numeric(outmat[,p])
              }
              outmat=as.matrix(outmat)
              input_names=row.names(m)
              input_names[1]="(Intercept)"
          ...
          warning("Moderation design type not meaningful for mean estimate type, change to type='association'!")
        }
      }else{
        if(Design[1,2]=="Splines"|Design[1,2]=="Powers"|Design[1,2]=="'Splines'"|Design[1,2]=="'Powers'"|
           Design[1,2]=="Any"|Design[1,2]=="'Any'"){
          ...
          drop2=unique(Design[2:nrow(Design),1])[unique(Design[2:nrow(Design),1])!=x_range]
          
          checker=rep(0,length(drop2))
          drop_row=rep(0,nrow(m))
          
          for(p in 2:nrow(m)){
            var_nam=Design[which(row.names(m)[p]==Design[,4]),1]
            if(var_nam==x_range){
              drop_row[p]=0
            }else if(var_nam%in%drop2){
              if(checker[which(drop2==var_nam)]==1){
                drop_row[p]=1
              }else{
                drop_row[p]=0
                checker[which(drop2==var_nam)]=1
              }
            }else{
              drop_row[p]=0
            }
          ...
          drop_rows=NULL
          for(p in 2:nrow(m)){
            if(Design[p,1]==x_range){
              drop_rows=c(drop_rows,p-1)
            }
          }
          
          dropper=NULL
          keeper=NULL
          for(p in 2:length(row.names(m))){
            if((p-1)%in%drop_rows){
              dropper=c(dropper,row.names(m)[p])
            }else{
              keeper=c(keeper,row.names(m)[p])
            }
          }          
          m=m[c(1,which(row.names(m)%in%keeper)),]
          
          
          add_rows=matrix(nrow=length(drop_rows),
                          ncol=Est_Num)
          
          
          out1="x_effects=list()"
          
          for(i in 1:length(drop_rows)){
            add_rows[i,]=rep(1,Est_Num)
            out1=c(out1,paste("x_effects[[",i,"]]=c('",Design[drop_rows[i]+1,4],"',",1,",",Design[drop_rows[i]+1,2],",",Design[drop_rows[i]+1,3],")",sep=""))
          }
          ...
          server <- function(input, output, session) {  
            data <- observeEvent(input$go, {
              ...
              outmat2=m1[2:nrow(m1),]
              
              q=1
              for(p in 1:nrow(outmat2)){
                if(row.names(outmat2)[p]%in%dropper){
                  outmat2[p,]=add_rows[which(row.names(outmat2)[p]==dropper),]
                }else{
                  outmat2[p,]=outmat[q,]
                  q=q+1
                }
              ...
              input_names=c(row.names(m1))
              input_names[1]="(Intercept)"
              
              
              Design2=Design
              for(p in 1:nrow(Design2)){
                Design2[p,3]=ifelse(Design2[p,1]==x_range,NA,Design2[p,3])
                Design2[p,1]=ifelse(Design2[p,1]==x_range,Design2[p,4],Design2[p,1])
              }
              
              L <<-L_pred_any(outmat2,
                              mod,
                              Design2,
                              input_names)
...
df_L=function(mod,
  ...
  glme4=ifelse("glmerMod"%in%c(class(mod)),1,0)
  glmmTMB=ifelse("glmmTMB"%in%class(mod),1,0)
  cox=ifelse("coxph"%in%class(mod),1,0)
  rma_mv=ifelse("rma"%in%class(mod),1,0)
  lm=ifelse("lm"%in%class(mod),1,0)
  glm=ifelse("glm"%in%class(mod),1,0)
  ...
  if(cox==1){
    ...
    for(i in 1:nrow(x)){
      x_i=X2[-i,]
      m2=coxph(formula(line),data=x_i)
      dfl[i,]=c(NA,(t(L)%*%mod$coefficients-t(L)%*%m2$coefficients)/t(L)%*%mod$coefficients)
      if(i%in%breaks){
  ...
  if(cox==1){
    dfl=cbind(c(model.frame(mod)[,1]),dfl[,2:ncol(dfl)])
  }
  else{
  ...
  if(is.null(Est_Names)){
    if(cox==1){
      colnames(dfl)=c(paste(names(model.frame(mod))[1],"_time",sep=""),paste(names(model.frame(mod))[1],"_event",sep=""),1:ncol(dfl2))
    }else{
    ...
    if(cox==1){
      colnames(dfl)=c(paste(names(model.frame(mod))[1],"_time",sep=""),paste(names(model.frame(mod))[1],"_event",sep=""),Est_Names)
    }else{
      colnames(dfl)=c(names(model.frame(mod))[1],paste(names(model.frame(mod))[1],"_LOO",sep=""),Est_Names)
    }
  }
  if(cox==1){
    blurb="Leave One Out R2 not able to be calculated for cox proportional hazard regression because there is no predicted hazard"
  }else{
...
#data is the name of a data object

#x_name is the x variable to be processed into splines

#knots is a vector of cut points between spline segments


X_Spline=function(data,
                  x_name,
                  knots){
  dat_nam=deparse(substitute(data))
  var_nam=deparse(substitute(x_name))
  if(length(knots)==1){
    printer=paste(dat_nam,"$",var_nam,"_spline=ifelse(",
                  dat_nam,"$",var_nam,"<",ifelse(knots<0,paste("(",knots,")",sep=""),knots),",0,",
                  dat_nam,"$",var_nam,"-",ifelse(knots<0,paste("(",knots,")",sep=""),knots),")",sep="")
  }else{
    print_mat=matrix(nrow=length(knots),ncol=1)
    for(k in 1:length(knots)){
      print_mat[k,]=paste(dat_nam,"$",var_nam,"_spline_",ifelse(knots[k]<0,paste("neg",abs(knots[k]),sep=""),knots[k]),"=ifelse(",
                          dat_nam,"$",var_nam,"<",ifelse(knots[k]<0,paste("(",knots[k],")",sep=""),knots[k]),",0,",
                          dat_nam,"$",var_nam,"-",ifelse(knots[k]<0,paste("(",knots[k],")",sep=""),knots[k]),")",sep="")
    }
    printer=NULL
    for(k in 1:length(knots)){
      printer=paste(printer,print_mat[k,],sep='\n')
    }
  }
  if(length(knots)==1){
    cat(cat("","","Copy and paste the following into R to backup your work!",
...
#Design is a matrix from L_designer with type="Splines"

#Intended to be used with L_Specify and Est_Mod for writing L matrix

L_pred_spline=function(L_x,
                       mod,
                       design,
                       Est_Names=NULL){
  
  L_spl=matrix(nrow=(nrow(design)-1),
               ncol=length(unique(design[2:nrow(design),1])))
  rownames(L_spl)=c(unique(design[2:nrow(design),3]))
  colnames(L_spl)=c(unique(design[2:nrow(design),1]))
  for(q in 1:ncol(L_spl)){
    count=1
    for(p in 1:nrow(L_spl)){
      if(design[(p+1),1]==colnames(L_spl)[q]){
        L_spl[p,q]=count
        count=count+1
      }else{
        L_spl[p,q]=0
  ...
  L_x1=matrix(nrow=length(L_inputs(mod)$Coefficient),ncol=ncol(L_x))
  for(e in 1:ncol(L_x1)){
    count=1
    for(p in 1:nrow(L_spl)){
      for(q in 1:ncol(L_spl)){
        if(L_spl[p,q]>0){
          if(L_spl[p,q]==1){
            L_x1[p,e]=L_x[p,e]
          }else if(L_spl[p,q]==max(L_spl[,q])){
            crit=as.numeric(design[(1+p),2])
            L_x1[p,e]=ifelse(L_x[count,e]<crit,0,L_x[count,e]-crit)
          }else{
            crit=as.numeric(design[(1+p),2])
            row_id=which(L_spl[,q]==L_spl[p,q]+1)
            crit2=as.numeric(design[(1+row_id),2])
            L_x1[p,e]=ifelse(L_x[count,e]<crit,0,
                             ifelse(L_x[count,e]<crit2,L_x[count,e]-crit,0))
          }
          if(max(L_spl[,q])==p){
            count=count+1
          }
...
#Design is a matrix from L_designer with type="Powers"

#Intended to be used with L_Specify and Est_Mod for writing L matrix

L_pred_curve=function(L_x,
                      mod,
                      design,
                      Est_Names=NULL){
      ...
      for(q in 1:ncol(L_spl)){
        if(L_spl[p,q]>0){
          if(L_spl[p,q]==1){
            ...
            pow=as.numeric(design[(1+p),2])
            L_x1[p,e]=L_x[count,e]^pow
          }
          if(max(L_spl[,q])==p){
            count=count+1
          }
...
#Design is a matrix from L_designer

#input_names is a vector of variable names for L_x connecting them to coefficients in mod

#Intended to be used with L_Specify and Est_Mod for writing L matrix



L_pred_any=function(L_x,
                    mod,
                    design,
                    input_names,
                    Est_Names=NULL){
  ...
  des=design
  des[1,]=c("(Intercept)",1,NA,"")
  
  L_x1=rbind(1,L_x)
  
  if(ncol(split_text)>1){
    L_main=matrix(nrow=nrow(split_text[split_text[,2]=="",]),
                  ncol=ncol(L_x))
  }else{
    L_main=matrix(nrow=length(split_text),
                  ncol=ncol(L_x))
  }
  
  for(q in 1:ncol(L_main)){
    for(p in 1:nrow(L_main)){
      nam_p=des[p,1]
      var_p=des[p,4]
      x_p=L_x1[which(nam_p==input_names),q]
      L_main[p,q]=ifelse(is.na(des[p,3]),x_p^as.numeric(des[p,2]),
                         ifelse(x_p<as.numeric(des[p,3]),0,(x_p-as.numeric(des[p,3]))^as.numeric(des[p,2])))
    }
  }
  
  L2=matrix(nrow=nrow(split_text),ncol=length(c(Est_Names)))
  
  for(e in 1:length(c(Est_Names))){
    split_num=matrix(L_main[,e])
    for(p in 1:nrow(L2)){
      coef=split_text[p,]
      mult=1
      for(q in 1:nrow(split_num)){
